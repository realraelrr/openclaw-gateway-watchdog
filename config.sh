...
load_watchdog_env_file() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$key" in
      OPENCLAW_HOME|WATCHDOG_STATE_DIR|WATCHDOG_LOG_DIR|WATCHDOG_ENV_FILE|STATE_FILE|WATCHDOG_DISABLE_FILE|WATCHDOG_LOCK_DIR|WATCHDOG_RUNTIME_TMP_DIR|LOG_FILE|WATCHDOG_DISPLAY_NAME|DISCORD_WATCHDOG_WEBHOOK_URL|NOTIFIER|FAIL_THRESHOLD|MAX_RESTART_FAILURES|COOLDOWN_SEC|POST_RESTART_RETRIES|POST_RESTART_SLEEP_SEC|OPENCLAW_BIN|NODE_BIN|WATCHDOG_ENABLED|FEISHU_BOT_APP_ID|FEISHU_BOT_APP_SECRET|FEISHU_BOT_CHAT_ID|FEISHU_BOT_RECEIVE_ID_TYPE|FEISHU_BOT_API_BASE)
